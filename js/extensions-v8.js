(function(){
  ...
  window.v8ToggleCompact=function(){var on=document.body.classList.toggle('cat-compact');S('hdv_viewmode',on?'compact':'rich');var b=$('v8-viewbtn');if(b){b.classList.toggle('active',on);b.innerHTML=on?'<i class="fa-solid fa-newspaper"></i> '+'Vue riche':'<i class="fa-solid fa-table-cells"></i> '+'Vue grille';}};
  ...
  window.v8Views=function(){
    var views=L('hdv_views',{});
    var h='<h2 style="font-family:var(--primary-serif);font-size:1.8rem;margin-bottom:6px;">'+'Vues enregistrees'+'</h2>';
    h+='<p style="font-size:.82rem;color:var(--sage-green);margin-bottom:14px;">'+'Sauvegardez vos filtres et tris favoris.'+'</p>';
    var keys=Object.keys(views);
    if(!keys.length)h+='<p style="font-style:italic;opacity:.7;">'+'Aucune vue enregistree.'+'</p>';
    else h+='<div style="display:flex;flex-direction:column;gap:8px;">'+keys.map(function(k){return '<div style="display:flex;gap:8px;align-items:center;justify-content:space-between;border:1px solid var(--glass-border);border-radius:10px;padding:8px 12px;"><span>'+esc2(k)+'</span><span style="display:flex;gap:6px;"><button class="btn-luxe" onclick="window.v8ApplyView(\''+encodeURIComponent(k)+'\')"><i class="fa-solid fa-play"></i> '+'Appliquer'+'</button><button class="btn-luxe" onclick="window.v8DeleteView(\''+encodeURIComponent(k)+'\')"><i class="fa-solid fa-trash"></i></button></span></div>';}).join('')+'</div>';
    h+='<div class="v7-actions-row" style="margin-top:16px;"><button class="btn-luxe" onclick="window.v8SaveView()"><i class="fa-solid fa-floppy-disk"></i> '+'Enregistrer la vue actuelle'+'</button></div>';
    if(typeof window.openModalHTML==='function')window.openModalHTML(h);
  };
  window.v8SaveView=function(){var name=prompt('Nom de la vue :');if(!name)return;name=name.trim();if(!name)return;var views=L('hdv_views',{});var g=function(id){var e=$(id);return e?e.value:'';};views[name]={fam:g('v7-f-fam'),type:g('v7-f-type'),tox:g('v7-f-tox'),zone:g('v7-f-zone'),sort:g('v7-sort'),q:(($('searchInput')||{}).value)||''};S('hdv_views',views);toast('Vue enregistree');window.v8Views();};
  window.v8ApplyView=function(k){k=decodeURIComponent(k);var v=L('hdv_views',{})[k];if(!v)return;var set=function(id,val){var e=$(id);if(e){e.value=val||'';}};set('v7-f-fam',v.fam);set('v7-f-type',v.type);set('v7-f-tox',v.tox);set('v7-f-zone',v.zone);set('v7-sort',v.sort);var si=$('searchInput');if(si){si.value=v.q||'';si.dispatchEvent(new Event('input',{bubbles:true}));}var ff=$('v7-f-fam');if(ff)ff.dispatchEvent(new Event('change',{bubbles:true}));if(typeof window.renderCatalog==='function')window.renderCatalog();if(typeof window.closeModal==='function')window.closeModal();toast('Vue appliquee : '+k);};
  ...
  function injectPhotos(id){
    ...
    var wrap=ce('div','v8-photoblock');wrap.style.marginTop='10px';
    wrap.innerHTML='<label style="font-family:var(--primary-sans);font-size:.7rem;text-transform:uppercase;letter-spacing:1px;color:var(--sage-green);display:block;margin-bottom:6px;">'+'Photos personnelles'+'</label><div class="v8-photos" id="v8-photos-'+id+'"></div><input type="file" id="v8-photofile-'+id+'" accept="image/*" style="display:none"><button class="btn-luxe" type="button" id="v8-photoadd-'+id+'"><i class="fa-solid fa-camera"></i> '+'Ajouter une photo'+'</button>';
    ...
    if(inp)inp.onchange=function(){
      ...
      resizeImage(f,700,0.7,function(durl){
        ...
        }).then(function(){renderPhotos(id);toast('Photo ajoutee');})
          .catch(function(){toast('Stockage photo impossible');});
      });
      inp.value='';
    };
  }
  ...
  function injectReminderExtras(){var body=$('v7-modal-body');if(!body||body.querySelector('.v8-remextra'))return;var row=ce('div','v7-actions-row v8-remextra');row.style.marginTop='14px';row.innerHTML='<button class="btn-luxe" onclick="window.v8ExportICS()"><i class="fa-solid fa-calendar-plus"></i> '+'Exporter (.ics)'+'</button><button class="btn-luxe" onclick="window.v8GardenPDF()"><i class="fa-solid fa-file-pdf"></i> '+'Jardin en PDF'+'</button><button class="btn-luxe" onclick="window.v8ShareGarden()"><i class="fa-solid fa-qrcode"></i> '+'Partager le jardin'+'</button>';body.appendChild(row);}
  window.__v8RemExtras=injectReminderExtras; /* v9 remplace openReminders : il réinjecte ces boutons via ce point d'entrée */
  window.v8ExportICS=function(){var j=L('hdv_journal',{});var adopted=getPlants().filter(function(p){return p.inGarden===true;});var dt=new Date();var ds=dt.getFullYear()+pad2(dt.getMonth()+1)+pad2(dt.getDate());var lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Herbier de Vie//FR','CALSCALE:GREGORIAN'];var n=0;adopted.forEach(function(p){var every=j[p.id]&&j[p.id].waterEvery;if(every&&every>0){n++;lines.push('BEGIN:VEVENT','UID:water-'+p.id+'@herbier','DTSTART;VALUE=DATE:'+ds,'RRULE:FREQ=DAILY;INTERVAL='+every,'SUMMARY:'+'Arroser '+icsEsc(p.nomFr),'DESCRIPTION:'+icsEsc(p.nomLat||''),'END:VEVENT');}});lines.push('END:VCALENDAR');if(!n){toast('Definissez un intervalle d\'arrosage d\'abord');return;}downloadText(lines.join('\r\n'),'herbier-arrosage.ics','text/calendar');toast(n+' rappel(s) exporte(s)');};
  ...
  window.wishToggle=function(id,ev){if(ev){ev.stopPropagation();ev.preventDefault();}var w=getWish();if(w[id])delete w[id];else w[id]=1;S('hdv_wishlist',w);reflectWishlist();if(window._v8WishOnly&&typeof window.renderCatalog==='function')window.renderCatalog();toast(w[id]?'Ajoute aux souhaits':'Retire des souhaits');};
  ...
  function ensureQuizCtrls(){var card=$('quizCard');if(!card||$('v8-quizctrls'))return;var fams=uniqSorted(getPlants().map(function(p){return p.famille;}));var ctr=ce('div','v8-quizctrls');ctr.id='v8-quizctrls';/* Ces deux listes déroulantes sont injectées hors du document initial et sans
   <label> : #v8-quizfam n'avait aucun nom accessible (violation axe critique
   select-name), #v8-level ne reposait que sur son title. */
ctr.innerHTML='<button id="v8-timed" class="'+(window._v8Timed?'on':'')+'" aria-pressed="'+(window._v8Timed?'true':'false')+'"><i class="fa-solid fa-stopwatch" aria-hidden="true"></i> '+'Chrono'+'</button>'+'<select id="v8-level" title="'+'Difficulte'+'" aria-label="Temps de reponse par question"><option value="25">'+'Facile (25s)'+'</option><option value="15" selected>'+'Moyen (15s)'+'</option><option value="8">'+'Rapide (8s)'+'</option></select>'+'<select id="v8-quizfam" aria-label="Limiter le quiz a une famille botanique"><option value="">'+'Toutes familles'+'</option>'+fams.map(opt).join('')+'</select>';card.parentNode.insertBefore(ctr,card);$('v8-timed').onclick=function(){window._v8Timed=!window._v8Timed;this.classList.toggle('on',window._v8Timed);this.setAttribute('aria-pressed',window._v8Timed?'true':'false');if(typeof window.newQuestion==='function')window.newQuestion();};$('v8-level').onchange=function(){window._v8QuizSecs=parseInt(this.value,10)||15;if(window._v8Timed&&typeof window.newQuestion==='function')window.newQuestion();};$('v8-quizfam').onchange=function(){window._v8QuizFam=this.value;if(typeof window.newQuestion==='function')window.newQuestion();};}
  ...
  function renderDashX(){var sec=$('dashSection');if(!sec)return;var box=$('v8-dashx');if(!box){box=ce('div','v8-dashx');box.id='v8-dashx';sec.appendChild(box);}
    var leit=L('hdv_leitner',{});var boxes=[0,0,0,0,0,0];getPlants().forEach(function(p){var b=(leit[p.id]&&leit[p.id].box)||0;if(b>5)b=5;boxes[b]++;});var labels=['Nouveau','1j','3j','7j','14j','30j'];
    var leitHTML='<div class="v8-leit">'+boxes.map(function(c,i){return '<div class="lv"><b>'+c+'</b><small>'+labels[i]+'</small></div>';}).join('')+'</div>';
    var mastered=boxes[4]+boxes[5];leitHTML+='<p style="margin-top:10px;font-size:.8rem;color:var(--sage-green);">'+'Cartes bien memorisees : '+'<b>'+mastered+'</b></p>';
  ...
  window.v8GardenPDF=function(){var area=$('printArea');if(!area){toast('Indisponible');return;}var list=getPlants().filter(function(p){return p.inGarden===true;});if(!list.length){toast('Aucune plante adoptee');return;}var html='';for(var i=0;i<list.length;i+=4){html+='<div class="pg">';for(var j=i;j<i+4;j++){if(j<list.length){var p=list[j];html+='<div class="pcard"><div class="pc-h"><span>'+esc2(p.nomFr)+'</span><small>'+esc2(p.type)+'</small></div><div class="pc-b"><div class="pc-row"><b>'+'Identite'+'</b><span><i>'+esc2(p.nomLat)+'</i> \u2014 '+esc2(p.famille)+'</span></div><div class="pc-row"><b>'+'Origine'+'</b><span>'+esc2(p.region)+'</span></div><div class="pc-row"><b>'+'Conserv.'+'</b><span>'+esc2(p.besoins)+'</span></div><div class="pc-row"><b>'+'Sensib.'+'</b><span>'+esc2(p.ennemis)+'</span></div></div></div>';}else{html+='<div class="pcard empty"></div>';}}html+='</div>';}area.innerHTML=html;window.print();};
  ...
  window.v8ShareGarden=function(){var link=gardenLink();var n=(link.split('hdvg=')[1]||'').split('.').filter(Boolean).length;
    var h='<h2 style="font-family:var(--primary-serif);font-size:1.8rem;margin-bottom:6px;">'+'Partager mon jardin'+'</h2>';
    h+='<p style="font-size:.82rem;color:var(--sage-green);margin-bottom:12px;">'+n+' espece(s) adoptee(s). Ce lien recree la selection a l\'ouverture.'+'</p>';
    h+='<input class="v8-share-link" id="v8-sharelink" readonly value="'+esc2(link)+'">';
    h+='<div class="v7-actions-row"><button class="btn-luxe" onclick="window.v8CopyLink()"><i class="fa-solid fa-copy"></i> '+'Copier le lien'+'</button></div>';
  ...
  function importGardenFromHash(){var h=location.hash||'';var mIdx=h.indexOf('hdvg=');if(mIdx<0)return;var raw=h.slice(mIdx+5);var nums={};raw.split('.').forEach(function(x){var n=parseInt(x,10);if(n>0)nums[n]=1;});var matched=getPlants().filter(function(p){var n=parseInt(String(p.id).replace(/[^0-9]/g,''),10);return n>0&&nums[n];});var cnt=matched.length;if(!cnt)return;var msg='Importer '+cnt+' espece(s) partagee(s) dans votre jardin ?';if(window.confirm(msg)){matched.forEach(function(p){p.inGarden=true;});try{if(typeof saveData==='function')saveData();}catch(e){}if(typeof window.renderCatalog==='function')window.renderCatalog();toast(cnt+' espece(s) importee(s)');}try{history.replaceState(null,'',location.pathname+location.search);}catch(e){location.hash='';}}
  ...
  function augmentToolbar(){
    ...
    var sort=$('v7-sort');if(sort&&!sort.querySelector('option[value="adopt"]')){var add=function(v,label){var o=document.createElement('option');o.value=v;o.textContent=label;sort.appendChild(o);};add('tox','Toxicite');add('zone','Emplacement');add('adopt','Date d\'adoption');}
    // groupe de boutons v8
    if(!$('v8-tb')){var grp=ce('span','v7-tb-group');grp.id='v8-tb';
      var view=ce('button','v8-tb-btn');view.id='v8-viewbtn';view.type='button';var compact=document.body.classList.contains('cat-compact');view.innerHTML=compact?'<i class="fa-solid fa-newspaper"></i> '+'Vue riche':'<i class="fa-solid fa-table-cells"></i> '+'Vue grille';view.classList.toggle('active',compact);view.onclick=window.v8ToggleCompact;
      var wish=ce('button','v8-tb-btn');wish.id='v8-wishbtn';wish.type='button';wish.innerHTML='<i class="fa-solid fa-star"></i> '+'Souhaits';wish.classList.toggle('active',!!window._v8WishOnly);wish.onclick=window.v8ToggleWish;
      var views=ce('button','v8-tb-btn');views.type='button';views.innerHTML='<i class="fa-solid fa-bookmark"></i> '+'Vues';views.onclick=window.v8Views;
